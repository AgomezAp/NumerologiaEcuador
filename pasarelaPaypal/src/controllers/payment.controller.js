  ...
  try {
    ...
    console.log("*************************************************",response.data.status);
        ...
        if (response.data.status === "COMPLETED") {
          ...
          return res.redirect(`https://numerologia-ecuador.com/result?status=COMPLETED&token=${approvalToken}`);
        } else {
          ...
          return res.redirect(`https://numerologia-ecuador.com/result?status=COMPLETED&token=${rejectToken}`);
        }
  } catch (error) {
    if (error.response) {
      ...
      if(error.response.status === 422){
        ...
        return res.redirect(`https://numerologia-ecuador.com/result?status=NOT_COMPLETED`);//cambiar esta url por una cuando se tenga el front que va a manejar errores 
      }
